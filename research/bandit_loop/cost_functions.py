...
__all__ = [
    ...
    "cost_func_tanh299",
    "cost_func_tanh2999",
    "cost_func_adversarial99",
    "cost_func_sway_plotting_delete"
]
...
def _adversarial_cost(states, C):
    """Keep pole down"""
    cos = states[...,3]
    return (cos+1)/2
...
def _cost_func_sway_onoff(states, C):
    if not isinstance(states, np.ndarray):
        states = np.array(states)
    cost = np.ones(len(states)) * C / 2
    cost[states[..., 3] < -0.98] = 0
    pos = np.ones(len(states)) * C / 2
    pos[np.abs(states[...,0]) < .1] = 0
    cost += pos
    if len(states.shape) == 1:
        return cost[0]
    return cost
cost_func_sway_plotting_delete = partial(_cost_func_sway_onoff, C=1)
...
cost_func_adversarial99 = partial(_adversarial_cost, C=C99)
...
if __name__ == "__main__":
    ...
    adversarial = _adversarial_cost(states, C=1)
    ...
    # ax.plot(x, adversarial, label="Adversarial Cost")
    lns = p1 + p2 + p3 + p4 + p5
